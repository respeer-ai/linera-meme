...
pub struct RegisterApplicationHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface>
{
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface>
    RegisterApplicationHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &AmsStateOperation) -> Self {
        let AmsStateOperation::RegisterApplication { metadata } = operation else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> Handler<(), AmsStateResponse>
    for RegisterApplicationHandler<R, S>
{
    async fn handle(
        &mut self,
