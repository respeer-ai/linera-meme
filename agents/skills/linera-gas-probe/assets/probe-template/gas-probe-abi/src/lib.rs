...
pub fn encode_payload(kind: PayloadKind) -> Vec<u8> {
    match kind {
        PayloadKind::Amount => bcs::to_bytes(&Amount::from_tokens(123_456)).unwrap(),
        PayloadKind::Account => bcs::to_bytes(&sample_account()).unwrap(),
        PayloadKind::AccountAmount => bcs::to_bytes(&AccountAmountPayload {
            account: sample_account(),
            amount: Amount::from_tokens(42),
        })
        .unwrap(),
        PayloadKind::PoolLikeSmall => bcs::to_bytes(&PoolLikeSmallPayload {
        ...
        .unwrap(),
        PayloadKind::Bytes32 => bcs::to_bytes(&vec![7u8; 32]).unwrap(),
        PayloadKind::Bytes128 => bcs::to_bytes(&vec![7u8; 128]).unwrap(),
        PayloadKind::Bytes512 => bcs::to_bytes(&vec![7u8; 512]).unwrap(),
        PayloadKind::Bytes2048 => bcs::to_bytes(&vec![7u8; 2048]).unwrap(),
    }
}
