...
@pytest.fixture
def mock_post(client: QueryClient):
    with patch.object(client._session, "post") as mock:
        yield mock


def test_query_posts_to_application_endpoint(client: QueryClient, mock_post) -> None:
...
def test_query_raises_on_graphql_errors(client: QueryClient, mock_post) -> None:
...
def test_query_latest_state_version(client: QueryClient, mock_post) -> None:
    mock_post.return_value = _response({"latestStateVersion": 3})

    assert client.query_latest_state_version("chain1", "app1") == 3


def test_query_state_applications(client: QueryClient, mock_post) -> None:
    mock_post.return_value = _response(
        {
            "stateApplications": [
...
def test_query_business_application_id_returns_id(client: QueryClient, mock_post) -> None:
...
def test_query_business_application_id_returns_none(client: QueryClient, mock_post) -> None:
...
def test_mutation_append_state(client: QueryClient, mock_post) -> None:
...
def test_mutation_handoff(client: QueryClient, mock_post) -> None:
