...
export const mergeLatestPointMaps = (
  ...
  incoming.forEach((incomingEntries, interval) => {
    ...
    const mergedEntries = new Map(existingEntries.map((entry) => [`${entry.token_0}:${entry.token_1}`, entry]))

    incomingEntries.forEach((entry) => {
      const key = `${entry.token_0}:${entry.token_1}`
...
export const buildKlineSubscriptionMessage = (token_0: string, token_1: string, interval: Interval) => ({
  action: 'subscribe',
  topic: 'kline',
  token_0,
  token_1,
