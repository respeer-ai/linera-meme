...
//! Shared helpers for pool integration tests that exercise the meme -> pool path.
...
/// Setup shared by pool integration tests that exercise the meme -> pool path.
...
impl ProxyMemeSetup {
        ...
        let swap_application_id = swap_chain
            .create_application::<SwapAbi, SwapParameters, SwapInstantiationArgument>(
                ...
                SwapInstantiationArgument {
                    pool_bytecode_id: pool_bytecode_id.forget_abi(),
                    pool_state_bytecode_ids: vec![StateBytecodeId {
                        version: 1,
                        module_id: proxy_chain
    ...
    pub async fn create_meme_application(
        ...
        let effective_initial_liquidity = initial_liquidity.or(Some(Liquidity {
            fungible_amount: DEFAULT_INITIAL_LIQUIDITY_FUNGIBLE,
            native_amount: DEFAULT_INITIAL_LIQUIDITY_NATIVE,
        }));
        let native_amount = effective_initial_liquidity
            .as_ref()
            .map(|l| l.native_amount)
                ...
                block.with_operation(
                    ...
                    ProxyOperation::CreateMeme {
                        ...
                        meme_parameters: MemeParameters {
                            ...
                            initial_liquidity: effective_initial_liquidity,
                            virtual_initial_liquidity,
        ...
        // Snapshot the existing meme application ids before this creation so we can
        // detect when the proxy has registered the new one (new ids may be prepended).
        ...
        let chain_description = block
            ...
            .filter_map(|(blob_id, blob)| {
                (blob_id.blob_type == BlobType::ChainDescription).then(|| {
                    bcs::from_bytes::<ChainDescription>(blob.content().bytes()).unwrap()
                })
            })
            .next()
            .expect("proxy should have created a meme chain");

        // The meme chain is owned by the proxy chain's owners, so use the proxy key pair
        // to sign blocks on it.
        ...
        // Drive the cross-chain message flow until the meme app is registered in proxy state.
        // Do not process swap_chain here; callers need the certificate from swap_chain
        // to derive the pool chain.
        for _ in 0..8 {
