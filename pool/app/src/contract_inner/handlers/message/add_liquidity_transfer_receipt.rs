...
use abi::pool::{AddLiquidityTransferReceipt, FundRequest, FundType, PoolMessage, PoolResponse};
    ...
    S: StateInterface,
{
    ...
    async fn validate_request(&self, request: &FundRequest) -> Result<(), HandlerError> {
        ...
        pool.validate_token(Some(token));
        Ok(())
    }
    ...
    async fn credit_request(&mut self, request: &FundRequest) -> Result<(), HandlerError> {
        self.state
            .credit_claimable(request.token, request.from, request.amount_in)
            .await
            .map_err(|error| HandlerError::ProcessError(error.into()))
    }
    ...
    S: StateInterface,
{
    async fn handle(
        ...
        self.validate_request(&self.receipt.request).await?;
