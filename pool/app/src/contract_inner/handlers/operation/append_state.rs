...
pub struct AppendStateHandler<R: ContractRuntimeContext + AccessControl, S: PublicStateBaseInterface>
{
    ...
    AppendStateHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolOperation) -> Self {
        let PoolOperation::AppendState { state_application_id } = op else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext + AccessControl, S: PublicStateBaseInterface>
    Handler<PoolMessage, PoolResponse> for AppendStateHandler<R, S>
{
    async fn handle(
        &mut self,
