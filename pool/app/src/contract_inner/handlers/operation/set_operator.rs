...
pub struct SetOperatorHandler<R: ContractRuntimeContext + AccessControl, S: PublicStateBaseInterface>
{
    ...
    SetOperatorHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolOperation) -> Self {
        let PoolOperation::SetOperator { new_operator } = op else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext + AccessControl, S: PublicStateBaseInterface>
    Handler<PoolMessage, PoolResponse> for SetOperatorHandler<R, S>
{
    async fn handle(
        &mut self,
