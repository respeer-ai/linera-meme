...
pub struct SetFeeToSetterHandler<
...
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    SetFeeToSetterHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolOperation) -> Self {
        let PoolOperation::SetFeeToSetter { account } = op else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    Handler<PoolMessage, PoolResponse> for SetFeeToSetterHandler<R, S>
{
    async fn handle(
        &mut self,
