...
pub struct CreditClaimableHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> CreditClaimableHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::CreditClaimable {
            token,
            owner,
            amount,
        } = op
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> Handler<(), PoolStateV1Response>
    for CreditClaimableHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        self.state
            .credit_claimable(self.token, self.owner, self.amount)
            .await
