...
pub struct SetPoolHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
    runtime: Rc<RefCell<R>>,
    state: S,
    pool: Pool,
}

impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> SetPoolHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::SetPool { pool } = op else {
