...
pub struct LiquidityHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
    runtime: Rc<RefCell<R>>,
    state: S,
    account: Account,
}

impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> LiquidityHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::Liquidity { account } = op else {
    ...
    for LiquidityHandler<R, S>
{
    async fn handle(
        ...
        let liquidity = self
            .state
            .liquidity(self.account)
            .await
            .map_err(|error| HandlerError::ProcessError(Box::new(error)))?;

        let mut outcome = HandlerOutcome::new();
        outcome.with_response(PoolStateV1Response::Liquidity(liquidity));
        Ok(Some(outcome))
    }
}
