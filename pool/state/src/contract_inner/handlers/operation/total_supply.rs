...
pub struct TotalSupplyHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
    runtime: Rc<RefCell<R>>,
    state: S,
}

impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> TotalSupplyHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::TotalSupply = op else {
    ...
    for TotalSupplyHandler<R, S>
{
    async fn handle(
        ...
        outcome.with_response(PoolStateV1Response::TotalSupply(total_supply));
        Ok(Some(outcome))
    }
}
