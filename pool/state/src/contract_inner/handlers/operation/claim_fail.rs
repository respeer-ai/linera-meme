...
pub struct ClaimFailHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> ClaimFailHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::ClaimFail {
            token,
            owner,
            amount,
        } = op
    ...
    for ClaimFailHandler<R, S>
{
    async fn handle(
        ...
        self.state
            .claim_fail(self.token, self.owner, self.amount)
            .await
