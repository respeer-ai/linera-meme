...
pub struct DebitClaimableHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> DebitClaimableHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::DebitClaimable {
            token,
            owner,
            amount,
        } = op
    ...
    for DebitClaimableHandler<R, S>
{
    async fn handle(
        ...
        self.state
            .debit_claimable(self.token, self.owner, self.amount)
            .await
