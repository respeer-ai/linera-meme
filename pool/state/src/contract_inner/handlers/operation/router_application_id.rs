...
pub struct RouterApplicationIdHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface>
{
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface>
    RouterApplicationIdHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::RouterApplicationId = op else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> Handler<(), PoolStateV1Response>
    for RouterApplicationIdHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        outcome.with_response(PoolStateV1Response::RouterApplicationId(
            router_application_id,
        ));
        Ok(Some(outcome))
    }
