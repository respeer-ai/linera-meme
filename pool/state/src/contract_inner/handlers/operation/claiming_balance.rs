...
pub struct ClaimingBalanceHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> ClaimingBalanceHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::ClaimingBalance { token, owner } = op else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> Handler<(), PoolStateV1Response>
    for ClaimingBalanceHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        let balance = self
            .state
            .claiming_balance(self.token, self.owner)
            .await
            .map_err(|error| HandlerError::ProcessError(Box::new(error)))?;

        let mut outcome = HandlerOutcome::new();
        outcome.with_response(PoolStateV1Response::ClaimingBalance(balance));
        Ok(Some(outcome))
    }
}
