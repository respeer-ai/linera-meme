...
pub struct BurnSharesHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> BurnSharesHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::BurnShares { from, amount } = op else {
    ...
    for BurnSharesHandler<R, S>
{
    async fn handle(
        ...
        self.state
            .burn_shares(self.from, self.amount)
            .await
