...
pub struct BuildTransactionHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
    runtime: Rc<RefCell<R>>,
    state: S,
    transaction: Transaction,
}

impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> BuildTransactionHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &PoolStateV1Operation) -> Self {
        let PoolStateV1Operation::BuildTransaction { transaction } = op else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> Handler<(), PoolStateV1Response>
    for BuildTransactionHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        let transaction = self
            .state
            .build_transaction(self.transaction.clone())
            .await
            .map_err(|error| HandlerError::ProcessError(Box::new(error)))?;

        let mut outcome = HandlerOutcome::new();
        outcome.with_response(PoolStateV1Response::Transaction(transaction));
        Ok(Some(outcome))
    }
}
