...
pub struct ReadHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> ReadHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &StateOperation) -> Self {
        let StateOperation::Read { namespace, key } = operation else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface>
    Handler<StateMessage, StateResponse> for ReadHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        let value = self
            .state
            .read(StateKey::new(self.namespace, slot, self.key.clone()))
            .await
            .map_err(|error| HandlerError::ProcessError(error.into()))?;

        let mut outcome = HandlerOutcome::new();
        outcome.with_response(StateResponse::Read(value));
        Ok(Some(outcome))
    }
}
