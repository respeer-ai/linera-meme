...
pub struct BatchWriteHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> BatchWriteHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &StateOperation) -> Self {
        let StateOperation::BatchWrite { namespace, writes } = operation else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface>
    Handler<StateMessage, StateResponse> for BatchWriteHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        let writes = self
            ...
            .iter()
            .cloned()
            .map(|(key, value)| (StateKey::new(self.namespace, slot, key), value))
            .collect();
