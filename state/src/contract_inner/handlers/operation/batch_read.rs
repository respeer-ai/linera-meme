...
pub struct BatchReadHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> BatchReadHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &StateOperation) -> Self {
        let StateOperation::BatchRead { namespace, keys } = operation else {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface>
    Handler<StateMessage, StateResponse> for BatchReadHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        outcome.with_response(StateResponse::BatchRead(values));
        Ok(Some(outcome))
    }
}
