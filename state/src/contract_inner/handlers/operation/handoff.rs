...
pub struct HandoffHandler<R: ContractRuntimeContext> {
...
impl<R: ContractRuntimeContext> HandoffHandler<R> {
    pub fn new(runtime: Rc<RefCell<R>>, operation: &StateOperation) -> Self {
        let StateOperation::Handoff {
            application_id,
            namespace,
            new_application_id,
        } = operation
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext> Handler<StateMessage, StateResponse> for HandoffHandler<R> {
    async fn handle(
        &mut self,
