...
pub struct WriteHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> WriteHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &StateOperation) -> Self {
        let StateOperation::Write {
            namespace,
            key,
            value,
        } = operation
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface>
    Handler<StateMessage, StateResponse> for WriteHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        self.state
            .write(
                StateKey::new(self.namespace, slot, self.key.clone()),
                self.value.clone(),
            )
            .await
            .map_err(|error| HandlerError::ProcessError(error.into()))?;
