...
pub struct UnfreezeNamespaceHandler<R: ContractRuntimeContext> {
    runtime: Rc<RefCell<R>>,
    application_id: ApplicationId,
}

impl<R: ContractRuntimeContext> UnfreezeNamespaceHandler<R> {
    pub fn new(runtime: Rc<RefCell<R>>, operation: &StateOperation) -> Self {
        let StateOperation::UnfreezeNamespace { application_id } = operation else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext> Handler<StateMessage, StateResponse>
    for UnfreezeNamespaceHandler<R>
{
    async fn handle(
        &mut self,
        ...
        outcome.with_message(target_chain_id, StateMessage::UnfreezeNamespace, false);
