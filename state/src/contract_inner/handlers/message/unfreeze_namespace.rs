...
pub struct UnfreezeNamespaceMessageHandler<R: ContractRuntimeContext, S: StateInterface> {
    runtime: Rc<RefCell<R>>,
    state: S,
}

impl<R: ContractRuntimeContext, S: StateInterface> UnfreezeNamespaceMessageHandler<R, S> {
...
impl<R: ContractRuntimeContext, S: StateInterface> Handler<StateMessage, StateResponse>
    for UnfreezeNamespaceMessageHandler<R, S>
{
    async fn handle(
        &mut self,
