...
fn business_state(runtime: &ServiceRuntime<ProxyService>) -> ProxyState {
...
fn query() {
    ...
    let runtime = Arc::new(
        ServiceRuntime::<ProxyService>::new().with_query_application_handler(
            move |_application_id, query| {
                let request: Request = serde_json::from_slice(&query).unwrap();
                if request.query.contains("memeBytecodeId") {
                    serde_json::to_vec(&Response::new(
                        Value::from_json(json!({"memeBytecodeId": meme_bytecode_id})).unwrap(),
                    ))
                    .unwrap()
                } else {
    ...
    let state = business_state(&runtime);
