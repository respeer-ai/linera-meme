...
use proxy::{
    contract_inner::handlers::HandlerFactory, state::adapter::ContractStateAdapter,
};
use runtime::{
    contract::ContractRuntimeAdapter,
    interfaces::contract::ContractRuntimeContext,
};
...
impl ProxyContract {
    ...
    pub async fn on_op(&mut self, op: &ProxyOperation) -> ProxyResponse {
        ...
        let state_adapter =
            ContractStateAdapter::new(state_runtime_context, self.state.clone());
