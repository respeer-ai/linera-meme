...
async fn proxy_create_meme_real_initial_liquidity_single_owner_test() {
    ...
    suite
        .create_proxy_application(vec![operator_1, operator_2], vec![proxy_owner])
        .await;
...
async fn run_create_meme_product_state_case(
    ...
    suite.create_swap_application().await;
    suite.create_proxy_application(vec![], vec![proxy_owner]).await;
