...
async fn proxy_create_meme_real_initial_liquidity_multi_owner_disable_mining_test() {
    ...
    suite
        .create_proxy_application(
            vec![operator_1, operator_2],
            vec![proxy_user_1, proxy_user_2, proxy_user_3],
        )
        .await;
...
async fn proxy_remove_genesis_miner_requires_second_operator_approval_test() {
    ...
    suite.create_swap_application().await;
    suite
        .create_proxy_application(vec![operator_1, operator_2, operator_3], vec![proxy_owner])
        .await;
