...
pub struct ProposeAddGenesisMinerHandler<
...
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    ProposeAddGenesisMinerHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, msg: &ProxyMessage) -> Self {
        let ProxyMessage::ProposeAddGenesisMiner { operator, owner } = msg else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    Handler<ProxyMessage, ProxyResponse> for ProposeAddGenesisMinerHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        self.state
            ...
            .map_err(Into::into)?;
        self.state
            .approve_add_genesis_miner(self.owner, self.operator)
            .await
            .map_err(Into::into)?;
