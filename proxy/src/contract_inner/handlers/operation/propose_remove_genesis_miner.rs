...
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    ProposeRemoveGenesisMinerHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &ProxyOperation) -> Self {
        let ProxyOperation::ProposeRemoveGenesisMiner { owner } = op else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    Handler<ProxyMessage, ProxyResponse> for ProposeRemoveGenesisMinerHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        outcome.with_message(
            destination,
            ProxyMessage::ProposeRemoveGenesisMiner {
                operator,
                owner: self.owner,
            },
        );
