    ...
    ApproveRemoveGenesisMinerHandler<R, S>
{
    pub fn new(runtime: Rc<RefCell<R>>, state: S, op: &ProxyOperation) -> Self {
        let ProxyOperation::ApproveRemoveGenesisMiner { owner } = op else {
...
#[async_trait(?Send)]
impl<R: ContractRuntimeContext + AccessControl + MemeRuntimeContext, S: StateInterface>
    Handler<ProxyMessage, ProxyResponse> for ApproveRemoveGenesisMinerHandler<R, S>
{
    async fn handle(
        &mut self,
        ...
        outcome.with_message(
            destination,
            ProxyMessage::ApproveRemoveGenesisMiner {
                operator,
                owner: self.owner,
            },
        );
