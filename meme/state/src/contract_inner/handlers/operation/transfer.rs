...
pub struct TransferHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> TransferHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &MemeStateV1Operation) -> Self {
        let MemeStateV1Operation::Transfer { from, to, amount } = operation else {
