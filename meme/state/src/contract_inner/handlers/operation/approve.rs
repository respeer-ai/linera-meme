...
pub struct ApproveHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> ApproveHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &MemeStateV1Operation) -> Self {
        let MemeStateV1Operation::Approve {
            origin,
            spender,
            amount,
        } = operation
    ...
    for ApproveHandler<R, S>
{
    async fn handle(
        ...
        self.state
            .approve(self.origin, self.spender, self.amount)
            .await
            .map_err(|error| HandlerError::ProcessError(error.into()))?;
