...
pub struct TransferFromHandler<R: ContractRuntimeContext + AccessControl, S: StateInterface> {
...
impl<R: ContractRuntimeContext + AccessControl, S: StateInterface> TransferFromHandler<R, S> {
    pub fn new(runtime: Rc<RefCell<R>>, state: S, operation: &MemeStateV1Operation) -> Self {
        let MemeStateV1Operation::TransferFrom {
            origin,
            from,
            to,
            amount,
    ...
    for TransferFromHandler<R, S>
{
    async fn handle(
        ...
        self.state
            .transfer_from(self.origin, self.from, self.to, self.amount)
            .await
            .map_err(|error| HandlerError::ProcessError(error.into()))?;
