...
class CandleFinalityScheduler:
    def __init__(
        ...
        *,
        ...
        market_watermark_repository=None,
        market_watermark_repository_factory=None,
        ...
        self.market_watermark_repository = market_watermark_repository
        self.market_watermark_repository_factory = market_watermark_repository_factory
        ...
        self._thread_market_watermark_repository = None
        self._retired_market_watermark_repository_ids = set()
        self._market_watermark_repository_lock = threading.Lock()
    ...
    def emit_due_events(self) -> None:
        ...
        market_watermark_repository = self._market_watermark_repository_for_worker()
        try:
            for pool in pool_catalog_repository.list_current_pool_views():
                ...
                market_watermark_ms = market_watermark_repository.load_pool_market_watermark_ms(pool_application)
                if market_watermark_ms is None:
                    continue
                for interval, bucket_ms in INTERVAL_BUCKET_MS.items():
                    finalized_bucket_start = market_watermark_ms // bucket_ms * bucket_ms - bucket_ms
            ...
            self._close_market_watermark_repository_if_retired(market_watermark_repository)
    ...
    def stop(self) -> None:
        ...
        if emit_running:
            ...
            self._retire_thread_market_watermark_repository()
        else:
            self._close_thread_pool_catalog_repository()
            self._close_thread_market_watermark_repository()
    ...
    def _market_watermark_repository_for_worker(self):
        if self.market_watermark_repository_factory is None:
            return self.market_watermark_repository
        with self._market_watermark_repository_lock:
            if self._thread_market_watermark_repository is None:
                self._thread_market_watermark_repository = self.market_watermark_repository_factory()
            return self._thread_market_watermark_repository
    ...
    def _close_thread_market_watermark_repository(self) -> None:
        repository = self._thread_market_watermark_repository
        if repository is None:
            return
        close = getattr(repository, 'close', None)
        if close is not None:
    ...
    def _retire_thread_market_watermark_repository(self) -> None:
        with self._market_watermark_repository_lock:
            repository = self._thread_market_watermark_repository
            if repository is None:
                return
            self._retired_market_watermark_repository_ids.add(id(repository))
            self._thread_market_watermark_repository = None
    ...
    def _close_market_watermark_repository_if_retired(self, repository) -> None:
        if repository is None:
            return
        with self._market_watermark_repository_lock:
            if id(repository) not in self._retired_market_watermark_repository_ids:
                return
            self._retired_market_watermark_repository_ids.remove(id(repository))
        close = getattr(repository, 'close', None)
        if close is not None:
            try:
                close()
